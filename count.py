from mylib.centroidtracker import CentroidTracker
from mylib.trackableobject import TrackableObject
from imutils.video import VideoStream
from imutils.video import FPS
from mylib import config, thread
import time
import numpy as np
import argparse, imutils
import time, dlib, cv2
from playsound import playsound

def run():

    
    ap = argparse.ArgumentParser()
    ap.add_argument("-p", "--prototxt", nargs='?',const="default",
        help="path to Caffe 'deploy' prototxt file",default="mobilenet_ssd/MobileNetSSD_deploy.prototxt")
    ap.add_argument("-m", "--model", nargs='?',const="default",
        help="path to Caffe pre-trained model",default="mobilenet_ssd/MobileNetSSD_deploy.caffemodel")
    ap.add_argument("-i", "--input", type=str,nargs='?',
        help="path to optional input video file",default="videos/example_01.mp4")
    ap.add_argument("-o", "--output", type=str,nargs='?',
        help="path to optional output video file",default="output/ab.avi")
    
    ap.add_argument("-c", "--confidence", type=float, default=0.4,
        help="minimum probability to filter weak detections")
    ap.add_argument("-s", "--skip-frames", type=int, default=30,
        help="# of skip frames between detections")
    ap.add_argument("-l","--limit",type=int,help="to set limit of people inside",default="3")
    args = vars(ap.parse_args())

   
    CLASSES = ["background", "aeroplane", "bicycle", "bird", "boat",
        "bottle", "bus", "car", "cat", "chair", "cow", "diningtable",
        "dog", "horse", "motorbike", "person", "pottedplant", "sheep",
        "sofa", "train", "tvmonitor"]

    
    net = cv2.dnn.readNetFromCaffe(args["prototxt"], args["model"])

    
    vs = cv2.VideoCapture(args["input"])

    writer = None

    W = None
    H = None

    center = CentroidTracker(maxDisappeared=40, maxDistance=50)
    track = []
    Objects = {}

    Frames = 0
    inwards = 0
    outwards = 0
    total = []
    
    empty=[]
    empty1=[]

    fps = FPS().start()

    if config.Thread:
        vs = thread.ThreadingClass(config.url)

    while True:
        
        frame = vs.read()
        frame = frame[1] if args.get("input", False) else frame
        if args["input"] is not None and frame is None:
            break

        
        frame = imutils.resize(frame, width = 500)
        rgb = cv2.cvtColor(frame, cv2.COLOR_BGR2RGB)

        
        if W is None or H is None:
            (H, W) = frame.shape[:2]

        
        if args["output"] is not None and writer is None:
            fourcc = cv2.VideoWriter_fourcc(*"mp4v")
            writer = cv2.VideoWriter(args["output"], fourcc, 30,
                (W, H), True)

        
        status = "Waiting"
        rects = []

        
        if Frames % args["skip_frames"] == 0:
            status = "Detecting"
            track = []

            
            blob = cv2.dnn.blobFromImage(frame, 0.007843, (W, H), 127.5)
            net.setInput(blob)
            detections = net.forward()

            
            for i in np.arange(0, detections.shape[2]):
                confidence = detections[0, 0, i, 2]

                
                if confidence > args["confidence"]:
                    
                    idx = int(detections[0, 0, i, 1])

                    
                    if CLASSES[idx] != "person":
                        continue

                    
                    box = detections[0, 0, i, 3:7] * np.array([W, H, W, H])
                    (startX, startY, endX, endY) = box.astype("int")


                    
                    tracker = dlib.correlation_tracker()
                    rect = dlib.rectangle(startX, startY, endX, endY)
                    tracker.start_track(rgb, rect)

                    
                    track.append(tracker)
        else:
            
            for tracker in track:
        
                status = "Tracking"

                
                tracker.update(rgb)
                pos = tracker.get_position()

                
                startX = int(pos.left())
                startY = int(pos.top())
                endX = int(pos.right())
                endY = int(pos.bottom())

                
                rects.append((startX, startY, endX, endY))
        
        cv2.line(frame, (0, H//2), (W, H//2), (0, 255, 0), 3)
        cv2.putText(frame, "-Prediction border - Entrance-", (10, H - ((i * 20) + 200)),
            cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 0), 1)

        
        objects = center.update(rects)

       
        for (objectID, centroid) in objects.items():
            
            to = Objects.get(objectID, None)

           
            if to is None:
                to = TrackableObject(objectID, centroid)

            
            else:
                
                y = [c[1] for c in to.centroids]
                direction = centroid[1] - np.mean(y)
                to.centroids.append(centroid)

                
                if not to.counted:
                   
                    if direction < 0 and centroid[1] < H:
                        outwards += 1
                        empty.append(outwards)
                        
                        to.counted = True
                        

                    
                    elif direction > 0 and centroid[1] > H//2:
                        inwards += 1
                        empty1.append(inwards)
                        to.counted = True
                        
                    total = []
                    
                    total.append(len(empty1)-len(empty))
                    
                    print("Total people inside:", total)
                    
                    
                    
                    
                    if sum(total) > 3:
                            cv2.putText(frame, "-ALERT: People limit exceeded-", (10, frame.shape[0] - 80),
                                cv2.FONT_HERSHEY_COMPLEX, 0.5, (0, 0, 255), 2)
                            playsound('stop.mp3')		


            
            Objects[objectID] = to

           
            text = "ID {}".format(objectID)
            cv2.putText(frame, text, (centroid[0] - 10, centroid[1] - 10),
                cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 2)
            cv2.circle(frame, (centroid[0], centroid[1]), 4, (255, 255, 255), -1)

        info=[
        ("Exit", outwards),
        ("Enter", inwards),
        ("Status", status),
        ]

        info2 = [
        ("Total people inside", total),
        ]

        
                #
        for (i, (k, v)) in enumerate(info):
            text = "{}: {}".format(k, v)
            cv2.putText(frame, text, (10, H - ((i * 20) + 20)), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 0, 0), 2)

        for (i, (k, v)) in enumerate(info2):
            text = "{}: {}".format(k, v)
            cv2.putText(frame, text, (265, H - ((i * 20) + 60)), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 255), 2)
        
        
        if writer is not None:
            writer.write(frame)

        
        cv2.imshow("Real-Time Monitoring/Analysis Window", frame)
        key = cv2.waitKey(1) & 0xFF

        
        if key == ord("q"):
            break

        
        Frames += 1
        fps.update()
    
    fps.stop()
    print("[INFO] elapsed time: {:.2f}".format(fps.elapsed()))
    print("[INFO] approx. FPS: {:.2f}".format(fps.fps()))

    if config.Thread:
        vs.release()

    cv2.destroyAllWindows()
run()
